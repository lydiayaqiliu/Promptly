...
Please migrate to a newer model. Visit https://docs.anthropic.com/en/docs/resources/model-deprecations for more information.`);
  ...
  // background.js
  var evaluationCriteriaPrompt = "";
  var enhancementCriteriaPrompt = "";
  var criteria = { minRounds: 1, maxRounds: 3 };
  var activeTabId = null;
  var criteriasReadyResolve;
  var criteriasReady = new Promise((resolve) => {
    criteriasReadyResolve = resolve;
  });
  (async () => {
    try {
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    (async () => {
      ...
      switch (message.type) {
        ...
        case "PROFILE_READY": {
          if (!pendingSession) {
            console.warn("[PE background.js] PROFILE_READY with no session");
            return;
          }
